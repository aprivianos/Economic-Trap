...
# ══════════════════════════════════════════════
            ...
            for feature in geojson["features"]:
                ...
                for polygon in coords_list:
                    ...
                    fig_ws_map.add_trace(go.Scattergeo(
                        ...
                        fillcolor="rgba(0,212,255,0.35)" if is_selected else "rgba(255,255,255,0.04)",
                        line=dict(color="#00d4ff" if is_selected else "rgba(255,255,255,0.1)",
                                  width=2.5 if is_selected else 0.8),
    ...
    with col_right:
        ...
        if "ws_result" in st.session_state:
            ...
            # Tentukan target 1 tingkat di atas klasifikasi asli
            if klas_asli == "LOW":
                target = LOM_THRESHOLD
                label_target = "Lower-Middle Income"
            elif klas_asli == "LOM":
                target = UPM_THRESHOLD
                label_target = "Upper-Middle Income"
            elif klas_asli == "UPM":
                target = HIGH_THRESHOLD
                label_target = "High Income"
            else:
                target = UPM_THRESHOLD
                label_target = "Upper-Middle Income"
