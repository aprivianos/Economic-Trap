...
def what_if_scenario(model, encoders, df: pd.DataFrame,
    ...
    klas_orig = klasifikasi_dari_pdrbkap(pdrbkap_orig) if "KLASIFIKASI" not in row.columns else str(row["KLASIFIKASI"].iloc[0])
    klas_scen = klasifikasi_dari_pdrbkap(pdrbkap_scen)

    delta_pdrbkap = pdrbkap_scen - pdrbkap_orig
    if delta_pdrbkap > 0 and pdrbkap_scen < UPM_THRESHOLD:
        tahun_tersisa = (UPM_THRESHOLD - pdrbkap_scen) / delta_pdrbkap
